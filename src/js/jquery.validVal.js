...
(function( $ )
{
	...
	function ValidVal( form, opts )		// form为要验证的表单的jquery对象  opts为自定义配置
	{
		this.form = form;
		this.opts = $.extend( true, {}, $.fn[ _PLUGIN_ ].defaults, opts );

		this._gatherValidation();	// 将默认配置和自定义配置全都加载到 this.opts.validations 中
	...
	function ValidValField( field, form )		// field为要验证字段的jquery对象 form为 validVal() 的对象
	{
		this.field	= field;
		this.form  	= form;
		...
		this._gatherValidations();		// 将页面中显示的配置信息push到 ValidValField函数的 对象的validations属性中，并移除相关显示配置属性
		this._bindEvents();		// 为当前元素绑定focus、blur、keyup、change等事件
	...
	ValidValField.prototype = {
		...
		validate: function( onEvent, fixPlaceholder )	// onEvent:某事件(onBlur等)是否进行验证  fixPlaceholder:是否清空占位符
		{
			var that = this;
		...
		_gatherValidations: function()
		{
			...
			if ( classes && classes.length )
			{
				...
				if ( start != -1 )
				{
					var corrcls = classes.substr( start ).split( ' ' )[ 0 ],		// 值为corresponding:xxxx
						corresp = corrcls.substr( corsp.length );					// 值为xxxx
		...
		_togglePlaceholderValue: function( toggle )		// 清空或恢复 提示符placeholder的值
		{
			if ( this.placeholderValue !== false )
			{
				if ( toggle == 'clear' )
